class TreeNode {
  constructor(values, comparator) {
    ...
    this.comparator = comparator; // 0 => compare by SAT, 1 => compare by GPA
  }
}

class Tree {
  constructor(values) {
    this.root = new TreeNode(values, 0);  // root node compares by SAT
  }
  insert(node, values, comp) {
    if (!node) return new TreeNode(values, comp);
    var r = node;
    var c = node.comparator;  // determine which value to compare
    var o = c === 0 ? 1 : 0; // complement of c
    if (values[c] > node.values[c]) {
      // when inserting a score compared by SAT, if the gpa is higher
